...
void    AccS ensor_Init(void);

#ifndef SENSOR_OPTIMIZE
/* Power up sensor */
void    AccS ensor_PwrUp(void);
/* Power down senor */
void    AccS ensor_PwrDn(void);
/* Make senor enter sleep mode */
#define AccS ensor_Sleep    AccS ensor_PwrDn
#ifndef SENSOR_HIDE_FEAT
/* Sensor self-test*/
void    AccS ensor_SelfTest(void);
/* Sensor bandgap test */
void    AccS ensor_BGTest(void);
/* Senor temperature out enable */
void    AccS ensor_TOEN(void);
#endif

/* Get acceleration data from sensor */
s8      AccS ensor_GetData(u8* reg, u16 *ax, u16 *ay, u16 *az);
...
/* Get acceleration data from sensor */
#define AccSensor_GetData(reg, ax, ay, az) do { \
    u8 data[5];                                 \
    I2C_WriteTo(SENSOR_ADDR);                   \
    I2C_PutByte(SENSOR_INTL_REG);               \
